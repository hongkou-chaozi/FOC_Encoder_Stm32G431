                     ...
                     rty_tABC[3], DW_currloop *localDW)
{
   ...
   *  Sum: '<S19>/Sum1'
   */
  rty_tABC[0] = (-(cos_g + rtb_algDD_o1) / rtu_Vdc + 0.5F) * 8000.0;
  rty_tABC[1] = (-(cos_g + rtb_algDD_o2) / rtu_Vdc + 0.5F) * 8000.0;
  rty_tABC[2] = (-(cos_g + sin_g) / rtu_Vdc + 0.5F) * 8000.0;
...
void Decoder_FOC_step(void)
{
  ...
  if (rtM->Timing.TaskCounters.TID[1] == 0) {
     ...
     */
    if (rtb_Sum > 10.0F) {
      rtb_DeadZone = rtb_Sum - 18.0F;

      /* Saturate: '<S180>/Saturation' */
      rtDW.Saturation = 18.0F;
    } else {
      if (rtb_Sum >= -18.0F) {
        rtb_DeadZone = 0.0F;
      } else {
